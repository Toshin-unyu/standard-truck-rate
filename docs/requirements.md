...
| 距離取得 | Google Maps Routes API（自社契約） |
| 標準運賃取得 | 全日本トラック協会 Supabase API |
...
| 項目 | 説明 |
|------|------|
| 標準的な運賃 | 全日本トラック協会基準の算定結果 |
| 赤帽運賃 | 比較用（軽貨物選択時） |
| 計算根拠 | 適用した料金表・割増率の明示 |

### 4.2 標準運賃取得（トラ協Supabase連携）
...
### 4.5 Google Maps API連携

| 項目 | 内容 |
|------|------|
| 使用API | Routes API（Essentials tier） |
| 契約 | 自社契約 |
| 用途 | 出発地・目的地間の距離(km)取得 |
| 月間無料枠 | 10,000リクエスト |
...
| 項目 | 仕様 |
|------|------|
| キャッシュ対象 | 出発地・目的地の組み合わせごとの距離(km) |
| キャッシュヒット時 | APIを呼ばずDBから取得 |
| 有効期限 | 無期限（道路距離は基本的に不変） |
...
### 7.1 akabou_distance_fares（赤帽距離制運賃）
...
### 7.4 akabou_area_surcharges（赤帽地区割増）
...
### 7.5 akabou_additional_fees（赤帽付帯料金）
